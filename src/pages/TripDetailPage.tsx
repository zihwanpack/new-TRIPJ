...
import { Calendar, DollarSign, EllipsisVertical, MapPin, Pencil, Plus, Trash } from 'lucide-react';
...
export const TripDetailPage = () => {
  ...
  const [{ data: tripDetail }, { data: allEvents }] = useSuspenseQueries({
    queries: [
      {
  ...
  const { data: usersByEmails } = useSuspenseQuery<User[]>({
  ...
  const handleDeleteTrip = () => deleteTripMutation.mutate();
  const editTripEventHandler = () => navigate(`/trips/${tripId}/edit`);
  const handleClosePage = () => {
    navigate(location.state?.from === 'my-trips' ? '/my-trips' : '/');
  };
  ...
  const renderContent = () => {
    ...
    return <TripEventList events={filteredEvents} tripId={tripIdNumber} />;
  };
    ...
    <div className="flex flex-col h-dvh overflow-hidden relative bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <section className="flex flex-col gap-3 mt-4 mx-4 flex-1 overflow-y-auto scrollbar-hide pb-10">
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <TripMemberList users={usersByEmails ?? []} />
        </div>
...
const TripMemberList = ({ users }: TripMemberListProps) => {
  const visibleUsers = users.slice(0, 3);
  const restUsers = Math.max(users.length - 3, 0);
...
const TripEventList = ({ events, tripId }: TripEventListProps) => {
  return (
